...
Currently avis-imgv is shipped with three(sRGB, Adobe RGB and Display P3) profiles. A profile is chosen based on the
...
After successfully executing a user action, we can chose to automatically run a function by specifying a callback. For
...
Shortcuts can be configured in the settings. Check examples/config.yaml for a an example and keys.txt for valid keys and
modifiers.
